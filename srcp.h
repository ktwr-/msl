...
/*スレッドを作る*/
int mythread_create(int prim,void (*func)(void *),void *args){
	mutex();
	int id = search_free_thread();
	thread_table[id].state = ALIVE;
	thread_table[id].prim = prim;
...
static void mythread_handler(int sig){
	if(num_thread > 1 && flag ==0 && thcr > 0){
		if(thread_table[current_thread].prim > thread_table[thcr].prim){
			int temp = thcr;
			thcr = 0
			swapcontext(&thread_table[current_thread],&thread_table[temp]);
		}
	}
}
